...
const getDaysMessage = (days) => {
  ...
  const getFilteredData = () => {
    ...
    // กรองไม่แสดงคนที่เกษียณไปแล้ว
    filtered = filtered.filter(person => calculateDaysToRetirement(person.retirement_date) > 0);
  ...
  return (
    <div className="dashboard-container" style={{ 
      ...
      {/* ตัวกรองและ header - แสดงเสมอเมื่อโหลดเสร็จ */}
      {!loading && (
        <div style={{
          backgroundColor: "#fff",
          borderRadius: "18px",
      ...
      {loading ? (
        ...
        <div style={{
          ...
          <div style={{ fontSize: "48px", marginBottom: "16px" }}>🔍</div>
          <div style={{ fontSize: "20px", color: "#6b7280", fontWeight: "600", marginBottom: "8px" }}>
            ไม่พบผู้ที่ใกล้เกษียณในปีนี้
          </div>
        </div>
      ) : (
        <>
              ...
              return (
                ...
                >
                  ...
                  <div style={{
                    ...
                    {daysToRetirement > 180 && `📅 เหลือ ${Math.ceil(daysToRetirement / 365)} ปี`}
                  </div>
                </div>
              );
            })}
